...
- `lib/main.dart` initializes Flutter, loads local data, and registers `DataProvider`.
...
The app currently assumes a REST API at:

```text
http://{serverIp}/api/
```

Default `serverIp` is hardcoded in `DataProvider`:

```dart
String? serverIp = '192.168.31.142:5000';
```
