...
exec 9>/run/lock/cashflow-deploy.lock
if ! flock -n 9; then
  echo 'Another CashFlow deployment is already running.' >&2
  exit 75
fi
...
source .env
set +a
: "${APP_ORIGIN:?APP_ORIGIN must be set in /opt/cashflow/.env}"
...
"${compose[@]}" exec -T postgres \
  pg_dump -U "${POSTGRES_USER:-cashflow}" -d "${POSTGRES_DB:-cashflow}" -Fc \
  >"$BACKUP_PATH"
test -s "$BACKUP_PATH"
